...
const Header = () => {
  ...
  return (
    <>
      ...
      >
        <div 
            className={`container mx-auto px-6 h-16 sm:h-20 flex justify-between items-center rounded-2xl sm:rounded-[2rem] border border-white/40 dark:border-white/10 shadow-lg backdrop-blur-xl bg-white/60 dark:bg-slate-900/60 transition-all duration-300 ${scrolled ? 'shadow-lg' : 'shadow-none border-transparent bg-transparent backdrop-blur-none'}`}
        >
            <nav className="flex justify-between items-center w-full">
              ...
              <div className="lg:hidden flex items-center gap-3">
                <button 
                    onClick={() => setIsSearchOpen(true)}
                    className="text-slate-800 dark:text-slate-200 p-2 rounded-full hover:bg-white/20"
                >
                    <SearchIcon className="w-5 h-5" />
                </button>
                <ThemeToggle />
